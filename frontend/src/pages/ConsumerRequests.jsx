...
import { ShoppingCart, MessageSquare, User, FileText, Edit2, Trash2, MapPin, Calendar, Package, DollarSign, TrendingUp, Bell, X, Save, Star, MessageCircle, CheckCircle, XCircle } from 'lucide-react';
...
export default function ConsumerRequests() {
  ...
  const confirmDeleteRequest = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://127.0.0.1:8000/api/v1/consumer/${requestToDelete._id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
  ...
  const fetchRequestBids = async (requestId) => {
    ...
    try {
      ...
      const response = await fetch(`http://127.0.0.1:8000/api/v1/farmerBid/${requestId}`, {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
  ...
  const handleAcceptBid = async (bid) => {
    ...
    try {
      ...
      const bidResponse = await fetch(`http://127.0.0.1:8000/api/v1/farmerBid/${bid._id}`, {
        method: 'PATCH',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
      ...
      console.log('Bid accepted successfully, now updating request quantity...');
      ...
      const requestResponse = await fetch(`http://127.0.0.1:8000/api/v1/consumer/${selectedRequest._id}`, {
        method: 'PATCH',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
  ...
  const handleRejectBid = async (bid) => {
    ...
    try {
      ...
      const response = await fetch(`http://127.0.0.1:8000/api/v1/farmerBid/${bid._id}`, {
        method: 'PATCH',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
        ...
        <div className="p-8">
                ...
                >
                  ...
                  <div className="relative h-32 bg-gradient-to-br from-blue-400 via-indigo-400 to-purple-400 overflow-hidden group-hover:from-orange-400 group-hover:via-red-400 group-hover:to-pink-400 transition-all duration-700">
                    ...
                    <div className="absolute top-3 left-3 group-hover:scale-110 transition-transform duration-300">
                      <div className={`px-3 py-1 text-xs font-bold rounded-xl shadow-lg border border-white border-opacity-30 backdrop-blur-sm flex items-center space-x-1 ${
                        request.status === 'Active' ? 'bg-gradient-to-r from-green-500 to-emerald-500 text-white group-hover:shadow-green-500/50 group-hover:shadow-xl' :
                        request.status === 'Pending' ? 'bg-gradient-to-r from-yellow-400 to-orange-400 text-white group-hover:shadow-yellow-500/50 group-hover:shadow-xl' :
                        'bg-gradient-to-r from-gray-400 to-gray-500 text-white group-hover:shadow-gray-500/50 group-hover:shadow-xl'
                      }`}>
                        <span className={request.status === 'Active' ? 'animate-pulse' : request.status === 'Pending' ? 'animate-spin' : ''}>
                    ...
                    <div className="space-y-2 mb-4">
                      <div className="flex items-center justify-between group-hover:transform group-hover:translate-x-2 transition-transform duration-300 delay-75">
                        <div className="flex items-center space-x-1.5 text-xs text-gray-600">
                          ...
                          <span className="font-medium">Qty: {request.product_quantity} {request.quantity_unit}</span>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center space-x-1.5 text-xs text-gray-600 group-hover:transform group-hover:translate-x-1 transition-transform duration-300 delay-100">
                        ...
                        <span className="font-medium">Price: ৳{request.price_per_unit}/{request.quantity_unit}</span>
                      </div>
                      ...
                      <div className="flex items-start space-x-1.5 text-xs text-gray-600 bg-gray-50 rounded-lg p-2 group-hover:bg-gradient-to-r group-hover:from-gray-50 group-hover:to-blue-50 group-hover:transform group-hover:translate-x-1 transition-all duration-300 delay-150">
                        ...
                        <span className="leading-relaxed break-words font-medium">
                          Needed by: {new Date(request.when).toLocaleDateString()}
                        </span>
                      </div>
                    </div>
          ...
          {!loading && !error && requests.length === 0 && (
            <div className="text-center py-16">
              ...
              <p className="text-gray-500 mt-2">Create your first product request to get started</p>
              <button 
                onClick={() => setIsNewRequestModalOpen(true)}
            ...
            <div className="p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
                <div className="bg-gray-50 rounded-lg p-4">
                  ...
                  <p className="text-xl font-bold text-gray-900">
                    {selectedRequest.product_quantity} {selectedRequest.quantity_unit}
                  </p>
                </div>
                ...
                <div className="bg-gray-50 rounded-lg p-4">
                  <div className="flex items-center gap-2 text-gray-700 mb-2">
                    ...
                    <span className="text-sm font-medium text-gray-500">Needed By</span>
                  </div>
                ...
                <div className="bg-gray-50 rounded-lg p-4">
                  ...
                  <p className="text-xl font-bold text-green-600">
                    ৳{(selectedRequest.product_quantity * selectedRequest.price_per_unit).toLocaleString()}
                  </p>
                </div>
              </div>
              ...
              {selectedRequest.request_description && (
                <div className="mb-6">
                  ...
                  <div className="bg-gray-50 rounded-lg p-4">
                    <p className="text-gray-700 leading-relaxed">{selectedRequest.request_description}</p>
                  </div>
                </div>
              )}
              ...
              {/* Farmer Bids Section */}
              <div className="mb-6">
                <h4 className="text-lg font-semibold text-gray-800 mb-3">Farmer Bids</h4>
                ...
                {bidsLoading ? (
                  ...
                  <div className="bg-gray-50 rounded-lg p-6 text-center">
                    ...
                    <p className="text-gray-600 text-lg font-medium">No bid has been placed yet</p>
                    <p className="text-gray-500 text-sm mt-1">Farmers will see your request and can place bids</p>
                  </div>
                ) : (
                  <div className="space-y-4 max-h-96 overflow-y-auto">
                    {requestBids.map((bid, index) => (
                      <div key={bid._id || index} className={`border rounded-lg p-4 hover:shadow-md transition-shadow ${
                        ...
                        <div className="flex items-start justify-between mb-3">
                          <div className="flex items-start gap-3">
                            <div className="w-10 h-10 bg-gradient-to-br from-green-400 to-green-600 rounded-full flex items-center justify-center text-white font-bold text-sm">
                              {bid.farmer_name ? bid.farmer_name.charAt(0) : 'F'}
                            </div>
                            <div>
                              <h5 className="font-semibold text-gray-900">{bid.farmer_name || 'Anonymous Farmer'}</h5>
                              <div className="flex items-center gap-2 mt-1">
                                {bid.farm_location && (
                                  <div className="flex items-center gap-1">
                                    <MapPin className="w-3 h-3 text-gray-400" />
                                    <span className="text-xs text-gray-600">{bid.farm_location}</span>
                                  </div>
                                )}
                              </div>
                            </div>
                        ...
                        <div className="grid grid-cols-2 gap-3 mb-3">
                          <div className="bg-gray-50 rounded p-2">
                            <p className="text-xs text-gray-500">Offered Quantity</p>
                            <p className="font-semibold text-gray-900">
                              {bid.quantity || 'N/A'} {selectedRequest.quantity_unit}
                            </p>
                          </div>
                          <div className="bg-gray-50 rounded p-2">
                            <p className="text-xs text-gray-500">Price per Unit</p>
                            <p className="font-semibold text-gray-900">৳{bid.price_per_unit || 'N/A'}</p>
                          </div>
                        </div>

                        {bid.message && (
                          <p className="text-gray-700 text-sm mb-3 bg-gray-50 p-2 rounded">{bid.message}</p>
                        )}
            ...
            <div className="p-6">
              ...
              <div className="mb-4">
                ...
                <div className="bg-gray-50 rounded-lg p-3">
                  <p className="font-semibold text-gray-800">{requestToDelete.product_name}</p>
                  <p className="text-sm text-gray-600">
                    Quantity: {requestToDelete.product_quantity} {requestToDelete.quantity_unit} • 
                    Price: ৳{requestToDelete.price_per_unit}
                  </p>
                </div>
              </div>
