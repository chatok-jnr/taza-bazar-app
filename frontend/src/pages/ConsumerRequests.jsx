...
export default function ConsumerRequests() {
  ...
  const handleCreateRequest = async (e) => {
    ...
    try {
      ...
      const response = await fetch(
        "https://taza-bazar-backend.onrender.com/api/v1/consumer",
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const handleEditRequest = (request) => {
    ...
    setEditRequestForm({
      ...
      when: request.when ? request.when.split("T")[0] : "", // Convert to YYYY-MM-DD format
  ...
  const handleAcceptBid = async (bid) => {
    ...
    try {
      ...
      console.log(
        `Hello = ${
          bid.quantity * bid.price_per_unit + consumerResponse.total_spent
        } `
      );
      ...
      if (consumerResponse.ok) {
        ...
        if (!consumerUpdateResponse.ok) {
          const errorData = await consumerUpdateResponse
            .json()
            .catch(() => ({}));
                  ...
                  <div className="p-4">
                    ...
                    <p className="text-gray-600 text-sm mb-4 line-clamp-2 leading-relaxed">
                      {request.request_description ||
                        `Request for ${request.product_quantity} ${request.quantity_unit} of ${request.product_name}`}
                    </p>
