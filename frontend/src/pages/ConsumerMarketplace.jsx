...
function ProductCard({ post }) {
  return (
    <div className="bg-white rounded-lg shadow-md overflow-hidden transition-all duration-300 hover:scale-105 hover:shadow-xl">
      ...
      <div className="p-5">
        <h3 className="text-xl font-bold text-gray-800 mb-2">{post.productName}</h3>
        <p className="text-sm text-gray-600 mb-3">by {post.farmer}</p>
        
        <div className="space-y-2 mb-4">
          <div className="flex items-center text-gray-700">
            <Package size={16} className="mr-2 text-green-600" />
            <span className="text-sm">Quantity: <strong>{post.quantity}</strong></span>
          </div>
...
export default function MarketplacePage() {
  ...
  // Filter logic
  const filteredPosts = farmerPosts.filter(post => {
    const categoryMatch = selectedCategory === 'All' || post.category === selectedCategory;
    const locationMatch = selectedLocation === 'All' || post.location === selectedLocation;
    
    let priceMatch = true;
    if (priceRange !== 'All') {
      const price = parseInt(post.price.replace(/[^0-9]/g, ''));
      if (priceRange === 'Under ৳50') priceMatch = price < 50;
      else if (priceRange === '৳50 - ৳80') priceMatch = price >= 50 && price <= 80;
