...
export default function ConsumerDashboard() {
  ...
  const [requests, setRequests] = useState([
    {
      ...
      adminDeal: true,
      bids: [
        { id: 1, farmerName: 'Ahmed Farm', price: '$2.30/kg', rating: 4.8, quantity: 200, response: 'pending' },
        { id: 2, farmerName: 'Green Valley', price: '$2.40/kg', rating: 4.5, quantity: 150, response: 'pending' }
      ]
    },
    {
      id: 2,
      ...
      adminDeal: false,
      bids: [
        { id: 3, farmerName: 'Harvest Fields', price: '$1.15/kg', rating: 4.9, quantity: 500, response: 'pending' }
      ]
    },
    {
      id: 3,
  ...
  const saveEdit = () => {
    setRequests(requests.map(r => r.id === editingRequestId ? { ...r, ...editForm } : r));
    cancelEdit();
  };
        ...
        <div className="p-8">
          ...
          <div className="space-y-6">
            {requests.map((request) => (
              <Link to={`/request/${request.id}`} key={request.id} className="block">
                <div className="bg-white rounded-lg shadow-md hover:shadow-xl transition-shadow duration-300">
                  <div className="p-6">
                    <div className="flex justify-between items-start">
                      <div className="flex-1">
                        <div className="flex items-center space-x-3 mb-4">
                          <h3 className="text-xl font-bold text-gray-800">{request.productName}</h3>
                          {request.adminDeal && (
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                          ...
                          <div className="flex items-center space-x-2 text-gray-700">
                            ...
                            <div>
                              ...
                              <p className="font-semibold">{request.pricePerUnit}</p>
                            </div>
                          </div>
                          <div className="flex items-center space-x-2 text-gray-700">
                      ...
                      <div className="flex space-x-2 ml-4">
                        <button
                          onClick={(e) => { e.preventDefault(); e.stopPropagation(); startEdit(request); }}
                          className="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition-colors duration-200"
                        >
                          <Edit2 size={20} />
                        </button>
                        <button 
                          onClick={(e) => { e.preventDefault(); e.stopPropagation(); handleDeleteRequest(request.id); }}
            ...
            <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
              <div className="bg-white rounded-lg w-full max-w-xl p-6">
                ...
                <div className="grid grid-cols-1 gap-3">
                  <input name="productName" value={editForm.productName} onChange={handleEditChange} className="border p-2 rounded" placeholder="Product name" />
                  <div className="grid grid-cols-3 gap-2">
                    <input name="quantity" value={editForm.quantity} onChange={handleEditChange} className="border p-2 rounded col-span-2" placeholder="Quantity" />
                    <input name="unit" value={editForm.unit} onChange={handleEditChange} className="border p-2 rounded" placeholder="Unit" />
                  </div>
                  <input name="pricePerUnit" value={editForm.pricePerUnit} onChange={handleEditChange} className="border p-2 rounded" placeholder="Price per unit" />
                  <input name="dateNeeded" value={editForm.dateNeeded} onChange={handleEditChange} className="border p-2 rounded" placeholder="Date needed" />
                  <input name="location" value={editForm.location} onChange={handleEditChange} className="border p-2 rounded" placeholder="Location" />
                  <div className="flex justify-end gap-2 mt-4">
                    <button onClick={cancelEdit} className="px-4 py-2 bg-gray-100 rounded">Cancel</button>
