...
export default function FarmerSidebar() {
  ...
  const sidebarItems = [
    ...
    { name: "My Listings", icon: FileText, path: "/farmer", tab: "My Listings" },
    { name: "New Listing", icon: Plus, path: "/farmer/new-listing" },
  ...
  const handleNavigation = (item) => {
    if (item.tab) {
      // For items that still need tab navigation (like My Listings)
      const searchParams = new URLSearchParams();
      searchParams.set('tab', item.tab);
      navigate(`${item.path}?${searchParams.toString()}`);
    } else {
  ...
  // Function to determine if a tab is active based on current path
